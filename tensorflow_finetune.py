...
parser.add_argument('--predict_dir', default='test-predict')
...
def prediction_unknow_class(sess, prediction, is_training, pred_init_op):
...
def main(args):
    ...
    with graph.as_default():
        ...
        def _parse_function(filename, label):
            ...
            #     black_image = tf.zeros((256,256),tf.float32)
        ...
        def training_preprocess(image, label, filename):
            ...
            return centered_image, label, filename

        # Preprocessing (for validation)
        # (3) Take a central 224x224 crop to the scaled image
        # (4) Substract the per color mean `VGG_MEAN`
        # Note: we don't normalize the data here, as VGG was trained without normalization
        def val_preprocess(image, label, filename):
        ...
        # Prediction dataset
        ##################################################################
        # pred_dataset = tf.contrib.data.Dataset.from_tensor_slices((pred_filenames, pred_labels))
        # pred_dataset = pred_dataset.map(_parse_function,
        #     num_threads=args.num_workers, output_buffer_size=args.batch_size)
        # pred_dataset = pred_dataset.map(val_preprocess,
        #     num_threads=args.num_workers, output_buffer_size=args.batch_size)
        # batched_pred_dataset = pred_dataset.batch(args.batch_size)
        ##################################################################
        ...
        # pred_init_op = iterator.make_initializer(batched_pred_dataset)
